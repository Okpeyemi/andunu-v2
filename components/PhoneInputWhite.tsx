...
interface PhoneInputWhiteProps {
  value: string;
...
export default function PhoneInputWhite({ value, onChange, className, placeholder }: PhoneInputWhiteProps) {
    ...
    <div className="flex-1 w-full">
      ...
      {(error || hint) && (
        <p className={`text-xs mt-2 px-2 ${
          error ? 'text-red-200 font-medium' : 'text-white/80'
        }`}>
          {error || hint}
        </p>
      )}
